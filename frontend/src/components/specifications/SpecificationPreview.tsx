...
export const SpecificationPreview: React.FC<SpecificationPreviewProps> = ({
  ...
  const [specification, setSpecification] = useState<SpecificationData | null>(
    null
  );
  ...
  useEffect(() => {
    loadSpecification();
  }, [conversationId, loadSpecification]);
  ...
  const handleCopyFile = async (fileType: FileType) => {
  ...
  const handleValidateSpecifications = async () => {
    if (!specification) return;
  ...
  const getTabTitle = (fileType: FileType): string => {
    const titles = {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Generated on{' '}
            {new Date(specification.metadata.generatedAt).toLocaleDateString()}{' '}
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center">
              ...
              <span className="ml-2 text-sm text-gray-600">
                ({Math.round(specification.validation.completeness)}% complete)
              </span>
            </div>
            <Button
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold">{getTabTitle(activeTab)}.md</h3>
          <div className="flex space-x-2">
            <Button
